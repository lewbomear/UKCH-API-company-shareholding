...
def generate_relevant_individual_info():
    ...
    exact_name_matches = [
        item for item in DATA["items"] if item["title"].lower() == OFFICER_NAME.lower()
    ]

    exact_name_dob_matches = [
        item for item in exact_name_matches if (
            isinstance(item.get("date_of_birth"), dict)
    ...
    for match in exact_name_dob_matches:
        ...
        current_officer_data = []  # Store data for the current officer
        ...
        while remaining_results > 0:
            ...
            current_officer_data.extend(current_data["items"])  # add current page's data to current officer's data
        ...
        officer_data_cache.append(current_officer_data)  # extend the cache with the current officer's data
    ...
    for appointment in merged_officer_data:
