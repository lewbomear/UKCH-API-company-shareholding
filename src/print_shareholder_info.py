...
def print_shareholder_info():
    ...
    for item in items:
        ...
        if 'confirmation-statement-with-updates' in description.lower():
            ...
            metadata_response = requests.get(metadata_link, auth=(api_key, ''), headers={'Accept': 'application/json'})
    ...
    pages = convert_from_path('confirmation_statement.pdf', 250, poppler_path=r'C:\Program Files\poppler-23.01.0\Library\bin')
    ...
    text = pytesseract.image_to_string(combined_image).replace('\n\n', '\n').replace('\n', '  ').replace('confirmation  statement', 'confirmation statement').replace('this  confirmation', 'this confirmation').replace('of  this confirmation', 'of this confirmation').replace('date  of this', 'date of this').replace(r'\b([A-Z][a-z]+)  ([A-Z][a-z]+)\b', r'\1 \2')
    ...
    shares = re.findall(r'(?<!0 )(\d+) ORDINARY shares held as at the date of this confirmation statement  Name: (\S+(?: \S+)*)', text)
